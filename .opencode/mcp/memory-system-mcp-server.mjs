...
const memoryClient = {
  ...
  getMemoryById: async (id) => {
    const response = await makeRequest(`${MEMORY_SERVICE_URL}/api/memory/${id}`, {
      method: 'GET'
    });
  ...
  deleteMemory: async (id) => {
    await makeRequest(`${MEMORY_SERVICE_URL}/api/memory/${id}`, {
      method: 'DELETE'
    });
  },
...
const chromaClient = {
  query: async (collection, query, n_results) => {
